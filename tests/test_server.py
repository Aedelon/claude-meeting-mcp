...
def test_check_status_keys():
    """Verify check_status returns all expected keys."""
    with (
        ...
        from claude_meeting_mcp.server import check_status
...
def test_configure_valid_key():
    """Verify configure updates config correctly."""
    from claude_meeting_mcp.server import configure
    ...
    with patch("claude_meeting_mcp.server.update_config") as mock_update:
        ...
        result = configure("whisper.model", "small")
        assert result["status"] == "updated"


def test_configure_invalid_key():
    """Verify configure returns error for invalid key."""
    from claude_meeting_mcp.server import configure
    ...
    with patch(
        ...
        result = configure("bad.key", "value")
